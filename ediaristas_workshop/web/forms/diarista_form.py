...
class DiaristaForm(forms.ModelForm):
    ...
    # Remove a obrigação de preenchimento do campo 'codigo_ibge' no formulário
    #codigo_ibge = forms.IntegerField(required=False)
    class Meta:
        model = Diarista
        # Inclui todos os campos para que sejam exibidos na tela
        #fields = '__all__'

        # Exclui um campo específico no formulário
        exclude = ('codigo_ibge',)
